...
--ON DELETE CASCADE - при удалении данных с одной таблицы автоматически удаляются связанные данные с другой. 

DELETE FROM attempt
WHERE date_attempt < '2020-05-01'; --WHERE DATEDIFF('2020.05.01',date_attempt)<0

SELECT * FROM attempt;
SELECT * FROM testing;

--15. Добавить в таблицу attempt информацию по всем недостающим попыткам сдачи каждого из студентов по каждому из предметов

INSERT INTO attempt (student_id, subject_id, date_attempt) 
SELECT student_id, subject_id, NOW()+1
FROM student
    CROSS JOIN subject
    LEFT OUTER JOIN attempt USING (student_id, subject_id)
